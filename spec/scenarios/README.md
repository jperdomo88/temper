...
Each scenario is a different *molecule* in the substrate's terms — a specific combination of verb, target, effects, context, and justification. Two scenarios can share an effect type (`RESOURCE.DAMAGE` appears in lawful defense and in mass harm) while constituting completely different governable actions because the surrounding molecular structure differs. The scenarios below show the substrate's machinery operating on those molecular differences.
...
| [`limits_multi_step_laundering.md`](limits_multi_step_laundering.md) | Each individual action passes pairwise CCD; the *combination* crosses the policy line. The Behavioral Dossier layer over accumulated tuples catches the pattern. Pairwise CCD alone does not. |
| [`limits_context_laundering.md`](limits_context_laundering.md) | The action and effects are consistent, but the *context fields* permitting the action are false, stale, or over-broad. Caught by the context-attestation layer (cross-referencing claimed context against authoritative system records) and by the dossier's pattern analysis. |
